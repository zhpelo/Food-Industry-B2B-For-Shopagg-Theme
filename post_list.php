...
<section class="food-section-sm">
    <div class="food-container">
        <nav class="food-breadcrumb"><a href="<?= url('/') ?>"><?= h(t('nav_home')) ?></a> / <span><?= h(t('blog')) ?></span></nav>
        <h1 class="food-title"><?= h($title) ?></h1>
        <p class="food-subtitle"><?= $currentCategory && !empty($currentCategory['description']) ? h($currentCategory['description']) : h(t('post_industry_insights')) ?></p>
    </div>
</section>
...
<section class="food-section-sm" style="padding-top:1rem;">
    <div class="food-container food-layout" style="grid-template-columns:300px 1fr;">
        <aside class="food-sidebar">
            <div class="food-side-head"><?= h(t('post_categories')) ?></div>
            <a href="<?= url('/blog') ?>" class="food-side-link <?= !$currentCategory ? 'is-active' : '' ?>"><?= h(t('post_all_articles')) ?></a>
                ...
                <?php foreach ($items as $item): ?>
                    <article class="food-post-item">
                        ...
                        <div class="food-card-body">
                            ...
                            <p><?= h(mb_substr(strip_tags((string)$item['summary']), 0, 150)) ?></p>
                            <div style="margin-top:.75rem;"><a class="food-btn food-btn-soft" href="<?= h($item['url']) ?>"><?= h(t('post_read_full')) ?></a></div>
                        </div>
                    </article>
                <?php endforeach; ?>
            <?php endif; ?>
